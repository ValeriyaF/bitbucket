...
public class ChatV2CometClient: ChatV2Client, GFayeClientDelegate {
    ...
    /// Request a chat session using the Chat V2 Comet API.
    ///
    /// - parameters:
    ...
    /// Resuming a chat session using the Chat V2 Comet API.
    ///
    /// - parameters:
    ///     - queue: Dispatch queue to perform the request.
    ...
    ///     - transcriptPosition: Transcript position; ignored for this API.
    ///
    /// - throws:
    ...
    private func sendRequest(request: ChatV2CometApiRequest) {
        ...
        self.queue.sync {
            if self.cometClient == nil {
                ...
                self.cometClient = GFayeClient(aGFayeURLString: cometUrl, channel: channel) { msg in
